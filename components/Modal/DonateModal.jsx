...
const DonateModal = ({setOpenDonation}) => {
  ...
  return (
    <div className="flexCenter fixed inset-0 z-10 bg-overlay-black animated fadeIn">
      <div className="relative flex flex-col items-center max-w-lg gap-4 p-6 rounded-md shadow-md sm:px-12 bg-[#1a1a1a] first:text-gray-100">
        <button 
          ...
          <p className="flex-1 text-center dark:text-gray-400">
            Your fund we will be utilized in the improvement in the Cryptoking, an open SOurce NFT Marketplace
          </p>
          <Input 
            inputType="input"  
            title="Amount" 
            placeholder="Amount"
            handleClick={(e) => setDonateAmount(e.target.value)}></Input>
            <button 
              onClick={(e)=> donate(donateAmount)}
              type="button"
